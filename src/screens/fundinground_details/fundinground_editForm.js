...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, changes = [], fundingRoundId, fundinground, numfields;

        fundinground = row['fundinground'];
        fundingRoundId = row['fundinground'].find((x) => x.type === 'keyid').value;
                ...
                if (changes.length > 0) {
                    ...
                    if (tmp.length > 0) {
                        ...
                        if (rslt.status) {
                            ...
                            if (Helper.IsNullValue(mapItem.navpropname)) fundingRoundId = rslt.id;
        ...
        // Add or Update FundingRound StartupLogoIcon
        changes = TrackChanges('fundinground');
        if (changes.length > 0 && changes.indexOf('StartupLogoIcon') > -1) {
            data = fundinground.find((x) => x.key === 'StartupLogoIcon');
            rslt = await Support.AddOrUpdateDocument(data);
            if (rslt.status) {
                let newImageId = parseInt(rslt.id);
                data = [
                    { key: "RoundId", value: parseInt(fundingRoundId) },
                    { key: "FundingRoundStartupLogoIcon", value: newImageId }
                ];
                rslt = await Support.AddOrUpdateFundingRound(data, dropDownOptions);
                if (!rslt.status) return;

                let newValues = fundinground.find((x) => x.key === 'StartupLogoIcon').value;
                newValues = { ...newValues, DocId: newImageId };
                row['fundinground'].find((x) => x.key === 'StartupLogoIcon').value = newValues;
                                // Update Back for next tracking purpose
                UpdateBackUp('fundinground');
        ...
        global.AlertPopup("success", "FundingRound is updated successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
