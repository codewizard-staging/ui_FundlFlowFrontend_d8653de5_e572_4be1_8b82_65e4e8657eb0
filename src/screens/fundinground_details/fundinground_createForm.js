...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, fundingRoundId, numfields;
        const mapItems = MapItems;

        let fundinground = row['fundinground'];

        numfields = Helper.GetAllNumberFields(fundinground);
        if (numfields.length > 0) Helper.UpdateNumberFields(fundinground, numfields);


        // Add Or Update FundingRound
        rslt = await Support.AddOrUpdateFundingRound(fundinground, dropDownOptions, ['StartupLogoIcon',]);
        if (rslt.status) {
            fundingRoundId = rslt.id;
        } else { return; }

        for (let i = 0; i < mapItems.length; i++) {
            // Add or Update the FundingRound and navigation entity if it is deos not exist
            let navItem = fundinground.find(x => x.uicomponent === mapItems[i].uicomponent);
            if (!Helper.IsJSONEmpty(navItem) && Helper.IsNullValue(navItem.value)) {
                ...
                if (rslt.status) {
                    data = [
                        { key: "RoundId", value: parseInt(fundingRoundId) },
                        { key: navItem.key, value: parseInt(rslt.id) }
                    ];
                    rslt = await Support.AddOrUpdateFundingRound(data, dropDownOptions);
                ...
                // Add StartupLogoIcon
                prodImages = fundinground.find((x) => x.key === 'StartupLogoIcon');
                if (prodImages && !Helper.IsNullValue(prodImages.value)) {
                    rslt = await Support.AddOrUpdateDocument(prodImages);
                    if (rslt.status) {
                        data = [
                            { key: "RoundId", value: parseInt(fundingRoundId) },
                            { key: "FundingRoundStartupLogoIcon", value: parseInt(rslt.id) }
                        ];
                        rslt = await Support.AddOrUpdateFundingRound(data, dropDownOptions);
                        if (!rslt.status) return;
                    } else { return; }
        ...
        global.AlertPopup("success", "FundingRound is created successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
